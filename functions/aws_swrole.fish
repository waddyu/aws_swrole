function aws_swrole -d "Switch AWS IAM Roles and start new session."
  ...
  getopts $argv | while read -l key value
    switch $key
      case _
         set dest_aws $value
      case o
        set out_exports true; set dest_aws $value
      case '*'
          set opt_error 1; and break
    end
  end
  ...
  if [ $out_exports ]
    ...
    set -x AWS_SESSION_TOKEN $sess_token

    fish
    # function fish_prompt
    #   echo "debug: fish_prompt color is $AWS_COLOR"
    #   echo "debug: fish_prompt target is $SWITCH_TARGET"
    #
    #   set_color $AWS_COLOR
    #   echo -e "\nSwitching: $SWITCH_TARGET"
    #   echo -n (pwd) '> '
    #   set_color normal
    # end
